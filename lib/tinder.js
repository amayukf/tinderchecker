...
export class TinderClient {
  ...
  calculateRiskScore(data) {
    ...
    let photosCount = data.photos_count || 0;
    if (typeof photosCount === 'string') {
      photosCount = parseInt(photosCount.replace(/\+/g, ''), 10) || 1;
    }

    if (!data.verified) {
      score -= 15;
      reasons.push("Account lacks ID/Selfie verification badge");
    }

    if (photosCount <= 1) {
      score -= 20;
      reasons.push("Only 1 photo uploaded (catfish probability elevated)");
    }

    const accountAge = String(data.account_age || "");
    if (accountAge.includes("d") && !accountAge.includes("y") && !accountAge.includes("m")) {
      const match = accountAge.match(/(\d+)/);
      if (match && parseInt(match[1], 10) < 14) {
        score -= 25;
        reasons.push("Newly registered account (< 14 days old)");
      }
    }

    let level, label, badge;
    if (score >= 85) {
      level = "🟢 Low Risk";
    ...
    return {
      ...
      reasons: reasons.length ? reasons : ["High verification signals", "Established account activity history"]
    };
  }
  ...
  async getProfileData(username) {
        ...
        if (response.status === 200) {
          ...
          if (data && typeof data === 'object') {
            ...
            if (data.birthDate || data.name || photosList.length > 0) {
              ...
              resDict.risk_analysis = this.calculateRiskScore(resDict);
              return resDict;
    ...
    const resDict = await this._scrapePublicProfile(username);
    if (resDict && typeof resDict === 'object') {
      resDict.risk_analysis = this.calculateRiskScore(resDict);
    }
    return resDict;
  }
  ...
  async pingEndpoints() {
    const results = {};
    const t = Date.now();
    const sign = md5("asd94test" + t);

    for (const apiEndpoint of this.apiEndpoints) {
      const domain = apiEndpoint.split("//")[1].split("/")[0];
      try {
        const start = Date.now();
        const res = await fetch(`${apiEndpoint}?user=test&t=${t}&sign=${sign}`, {
          method: "GET",
          headers: this.headers,
        });
        const latency = Date.now() - start;
        if (res.status === 200) {
          results[domain] = `🟢 Online (${latency}ms)`;
        } else {
          results[domain] = `🟡 HTTP ${res.status}`;
        }
      } catch (err) {
